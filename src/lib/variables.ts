...
function buildStudentsTable(students: GenerationContext["students"]): string {
  ...
  return `<table style="border-collapse:collapse;width:100%;font-size:12px"><thead><tr>` +
    `<th style="border:1px solid #333;padding:4px 8px">№</th><th style="border:1px solid #333;padding:4px 8px">ФИО</th>` +
    `<th style="border:1px solid #333;padding:4px 8px">Паспорт</th><th style="border:1px solid #333;padding:4px 8px">СНИЛС</th>` +
    `</tr></thead><tbody>${rows}</tbody></table>`;
}

function buildStudentListRows(students: GenerationContext["students"], ctx: GenerationContext): string {
  return students.map((s, i) =>
    `<tr><td style="border:1px solid #333;padding:4px">${i + 1}</td>` +
    ...
    `<td style="border:1px solid #333;padding:4px">${ctx.group.program_hours}</td>` +
...
function buildRegistrationRows(students: GenerationContext["students"], ctx: GenerationContext, orderNum: string): string {
  return students.map((s, i) =>
    `<tr><td style="border:1px solid #333;padding:3px;text-align:center">${i + 1}</td>` +
    `<td style="border:1px solid #333;padding:3px">Удостоверение о ПК. ${esc(ctx.organization.name)}</td>` +
    `<td style="border:1px solid #333;padding:3px">${esc(ctx.group.program_title)}. Группа ${esc(ctx.group.number)}</td>` +
    `<td style="border:1px solid #333;padding:3px"></td><td style="border:1px solid #333;padding:3px"></td>` +
...
export function buildVariables(ctx: GenerationContext, opts: BuildVariablesOptions = {}): Record<string, string> {
  ...
  if (ctx.extras) {
    for (const [k, v] of Object.entries(ctx.extras)) vars[k] = String(v);
  }
  return vars;
}
...
export function renderTemplate(
  ...
  rawKeys: string[] = ["students_table", "students_list_rows", "registration_rows"]
): string {
  const raw = new Set(rawKeys);
  return html.replace(/\{\{\s*(&)?\s*([a-zA-Z0-9_]+)\s*\}\}/g, (match, isRaw, key) => {
...
export function findMissing(html: string, variables: Record<string, string>): string[] {
  const keys = new Set<string>();
  for (const m of html.matchAll(/\{\{\s*&?\s*([a-zA-Z0-9_]+)\s*\}\}/g)) keys.add(m[1]);
